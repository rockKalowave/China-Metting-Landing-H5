...
export function getStoredMiniAppUser() {
  return normalizeMiniAppUser(sessionStorage.getItem(MINIAPP_USER_STORAGE_KEY)
    ? parseJson(sessionStorage.getItem(MINIAPP_USER_STORAGE_KEY))
    : null);
}

export function cacheMiniAppUser(user) {
  const normalized = normalizeMiniAppUser(user);
  if (!normalized) {
    return null;
  }

  sessionStorage.setItem(MINIAPP_USER_STORAGE_KEY, JSON.stringify(normalized));
  return normalized;
}
...
export async function resolveMiniAppUser() {
  ...
  if (userFromUrl) {
    return cacheMiniAppUser(userFromUrl);
  }
  ...
  if (globalUser) {
    return cacheMiniAppUser(globalUser);
  }
